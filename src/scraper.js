...
const CRAIGSLIST_CITIES = {
        ...
        "los angeles": "losangeles", "new york": "newyork",
        "san francisco": "sfbay", seattle: "seattle", portland: "portland",
        chicago: "chicago", houston: "houston", phoenix: "phoenix",
        denver: "denver", miami: "miami", atlanta: "atlanta",
        boston: "boston", dallas: "dallas", detroit: "detroit",
        minneapolis: "minneapolis", austin: "austin",
        "san diego": "sandiego", "las vegas": "lasvegas",
};
...
function scrapeCraigslist(html, location) {
        ...
        results.forEach((el, i) => {
                  ...
                  if (title) {
                              deals.push({
                                            id: `cl-${i}-${Date.now()}`, title, price,
                                            priceText: priceText || "No price", image: null,
                                            url: href.startsWith("http") ? href : `https://craigslist.org${href}`,
                                            marketplace: "craigslist", location: location || "Local",
...
function buildEbayUrl(query) {
        return `https://www.ebay.com/sch/i.html?_nkw=${encodeURIComponent(query)}&_sop=10&LH_BIN=1&rt=nc`;
}
...
function scrapeEbay(html) {
        ...
        items.forEach((el, i) => {
                  ...
                  if (title && title !== "Shop on eBay") {
                              deals.push({
                                            id: `eb-${i}-${Date.now()}`, title, price,
                                            priceText: priceText || "See listing", image: img,
                                            url: href, marketplace: "ebay", location: "eBay",
                                            postedAt: new Date().toISOString(),
                              });
                  }
        });
...
function scrapeAutoTrader(html, location) {
        ...
        items.forEach((el, i) => {
                  ...
                  if (title && title.length > 3) {
                              deals.push({
                                            id: `at-${i}-${Date.now()}`, title, price,
                                            priceText: priceText || "Contact for price", image: img,
                                            url: href.startsWith("http") ? href : `https://www.autotrader.ca${href}`,
                                            marketplace: "autotrader", location: location || "Vancouver",
                                            postedAt: new Date().toISOString(),
                              });
                  }
        });
        if (deals.length === 0) {
                  ...
                  titles.forEach((title, i) => {
                              if (title && title.length > 3) {
                                            ...
                                            deals.push({
                                                            id: `at-fb-${i}-${Date.now()}`, title, price: p,
                                                            priceText: pt, image: null, url: "https://www.autotrader.ca",
                                                            marketplace: "autotrader", location: location || "Vancouver",
                                                            postedAt: new Date().toISOString(),
                                            });
                              }
...
const KIJIJI_LOCATIONS = {
        vancouver: "b-buy-sell/vancouver/", toronto: "b-buy-sell/city-of-toronto/",
        montreal: "b-buy-sell/ville-de-montreal/", calgary: "b-buy-sell/calgary/",
        edmonton: "b-buy-sell/edmonton/", ottawa: "b-buy-sell/ottawa/",
        winnipeg: "b-buy-sell/winnipeg/", hamilton: "b-buy-sell/hamilton/",
        kitchener: "b-buy-sell/kitchener-waterloo/", london: "b-buy-sell/london/",
        victoria: "b-buy-sell/victoria-bc/", halifax: "b-buy-sell/city-of-halifax/",
        saskatoon: "b-buy-sell/saskatoon/", regina: "b-buy-sell/regina/",
...
function scrapeKijiji(html, location) {
        ...
        const deals = [];
        const selectors = [
                  "[data-testid='listing-card']", ".search-item", "[data-listing-id]",
                  ".resultContainer", ".info-container", "[class*='ListItem']", "[class*='listing']",
                ];
        let items = [];
        ...
        if (items.length === 0) {
                  ...
                  allLinks.forEach((a, i) => {
                              ...
                              if (title && title.length > 5 && title.length < 200) {
                                            deals.push({
                                                            id: `kj-${deals.length}-${Date.now()}`, title,
                                                            price: null, priceText: "See listing", image: null,
                                                            url: href.startsWith("http") ? href : `https://www.kijiji.ca${href}`,
                                                            marketplace: "kijiji", location: location || "Vancouver",
                                                            postedAt: new Date().toISOString(),
                                            });
                              }
                  });
...
function buildFacebookSearchUrl(query, location) {
        const FB_CITIES = {
                  ...
                  portland: "portland", chicago: "chicago", houston: "houston",
                  phoenix: "phoenix", denver: "denver", miami: "miami",
                  atlanta: "atlanta", boston: "boston", dallas: "dallas", detroit: "detroit",
        };
...
function getFacebookDeals(query, location) {
        ...
        return [{
                  ...
                  price: null, priceText: "View on Facebook", image: null,
                  url: searchUrl, marketplace: "facebook",
                  location: location || "Local",
                  postedAt: new Date().toISOString(), isRedirect: true,
        }];
}

// ============ REVERB (REST API - Location Aware) ============

const REVERB_REGION_MAP = {
...
const CITY_TO_COUNTRY = {
        ...
        "london": "GB", "manchester": "GB", "birmingham": "GB", "edinburgh": "GB", "glasgow": "GB", "liverpool": "GB",
        "paris": "FR", "lyon": "FR", "marseille": "FR", "toulouse": "FR",
        "berlin": "DE", "munich": "DE", "hamburg": "DE", "frankfurt": "DE", "cologne": "DE",
        "sydney": "AU", "melbourne": "AU", "brisbane": "AU", "perth": "AU", "adelaide": "AU",
        "toronto": "CA", "vancouver": "CA", "montreal": "CA", "calgary": "CA", "edmonton": "CA", "ottawa": "CA", "winnipeg": "CA", "halifax": "CA",
        "new york": "US", "los angeles": "US", "chicago": "US", "houston": "US", "phoenix": "US",
        "san francisco": "US", "seattle": "US", "miami": "US", "boston": "US", "dallas": "US",
        "denver": "US", "atlanta": "US", "detroit": "US", "portland": "US", "austin": "US",
        "las vegas": "US", "san diego": "US", "minneapolis": "US",
...
const REGION_CURRENCY = {
        ...
        JP: "JPY", IT: "EUR", ES: "EUR", NL: "EUR", BE: "EUR", AT: "EUR",
        CH: "CHF", SE: "SEK", NO: "NOK", DK: "DKK", FI: "EUR", IE: "EUR",
        PT: "EUR", PL: "PLN", MX: "MXN", BR: "BRL", NZ: "NZD", SG: "SGD",
        HK: "HKD", IN: "INR", KR: "KRW", CN: "CNY", TW: "TWD", TH: "THB",
        PH: "PHP", ID: "IDR", MY: "MYR", ZA: "ZAR", IL: "ILS", AE: "AED",
        SA: "SAR", AR: "ARS", CL: "CLP", CO: "COP",
};

function getReverbRegion(location) {
        if (!location) return { region: null, currency: null };
        const loc = location.toLowerCase().trim();
        const directCode = REVERB_REGION_MAP[loc];
        if (directCode) {
                  return { region: directCode, currency: REGION_CURRENCY[directCode] || "USD" };
        }
        const cityCode = CITY_TO_COUNTRY[loc];
        if (cityCode) {
                  return { region: cityCode, currency: REGION_CURRENCY[cityCode] || "USD" };
        }
        return { region: null, currency: null };
}

async function fetchReverbListings(query, location) {
        const { region, currency } = getReverbRegion(location);
        const controller = new AbortController();
        ...
        try {
                  ...
                  if (data.listings && Array.isArray(data.listings)) {
                              data.listings.forEach((item, i) => {
                                            ...
                                            const shopCountry = item.shipping?.local?.rates?.[0]?.region || region || "";
                                            if (title) {
                                                            deals.push({
                                                                              id: `rv-${item.id || i}-${Date.now()}`,
                                                                              title: condition ? `${title} (${condition})` : title,
                                                                              price, priceText, image: photo, url: webLink,
                                                                              marketplace: "reverb",
                                                                              location: shopName || (region ? `Reverb ${region}` : "Reverb"),
...
export async function searchMarketplaces(query, marketplaces, location) {
  ...
  if (marketplaces.includes("craigslist")) {
        ...
        if (marketplaces.includes("reverb")) {
                  tasks.push(
                              fetchReverbListings(query, location)
                                .then((deals) => { allDeals.push(...deals); })
